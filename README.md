...
**Provider chain:** Groq whisper-large-v3 → local Whisper HTTP server

| Provider | Set in `.env` | Notes |
|---|---|---|
| **Groq Whisper** | `GROQ_API_KEY` | Primary; ~200ms, free tier |
| **Local Whisper** | `WHISPER_URL` | HTTP ASR server (e.g. separate Docker container); offline fallback |
...
| Voice ASR | [Groq](https://console.groq.com) (whisper-large-v3) + local Whisper HTTP (offline fallback) |
| Voice TTS | Yandex SpeechKit + Piper (local) + Kokoro-82M (local) + Groq |
