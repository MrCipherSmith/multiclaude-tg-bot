 ...
 *
 * Level 1: group buttons (Session, Memory, Projects, …)
...
function groupsKeyboard(): InlineKeyboard {
  const kb = new InlineKeyboard();
  GROUPS.forEach((g, i) => {
    kb.text(g.label, `menu:g:${g.id}`);
    if (i % 2 === 1) kb.row();
  });
  if (GROUPS.length % 2 !== 0) kb.row();
...
export async function handleMenu(ctx: Context): Promise<void> {
  await ctx.reply("Choose a category:", { reply_markup: groupsKeyboard() });
}
...
export async function handleMenuCallback(ctx: Context): Promise<void> {
  ...
  if (rest === "home") {
    ...
    await ctx.editMessageText("Choose a category:", { reply_markup: groupsKeyboard() });
    return;
  }
  ...
  if (rest.startsWith("g:")) {
    ...
    await ctx.answerCallbackQuery();
    await ctx.editMessageText(`${group.label}`, { reply_markup: commandsKeyboard(group) });
    return;
  }
...
// ── Command dispatcher ─────────────────────────────────────────────────────
// Lazy imports so we don't form circular dep chains.
